...
#########################
###  CMS/Util.pm
###  Version : $Id: Util.pm,v 1.1 2008/03/10 19:37:10 dave Exp $
###
...
sub get_sql_date {
  ...
  ###  Fudge over...
  if ( @_ == 1 ) {
    ###  If they give us an SQL date
    if ( $year =~ /^(\d{4})-(\d{2})-(\d{2})(?: (\d{1,2}):(\d{2}):(\d{2}))?$/s
         ) { ( $year, $month, $mday, $hour, $min, $sec ) = ( $1, $2, $3, $4 || 0, $5 || 0, $6 || 0 ); }
  ...
  ###  Reformat month
  my @months = ( qr[^jan], qr[^feb], qr[^mar], qr[^apr], qr[^may], qr[^jun], qr[^jul], qr[^aug], qr[^sep], qr[^oct], qr[^nov], qr[^dec] );
  foreach my $i ( 0..$#months ) { $month = $i + 1 if lc($month) =~ $months[$i]; }

  ###  Final Adjustments
  $year += 1900 if $year < 1900;
  $year += 100 if $year < 1970; # Y2K workaround
  $mday ||= 1;
...
Odyc::Util - Lib for shared utility functions

=head1 SYNOPSIS

    use Odyc::Util qw(&split_delim_line &join_delim_line);
    ...
    use Odyc::Util qw(&read_conf &write_conf);
    ...
    use Odyc::Util qw(&do_validation);

    my ( $ok, $newval, $error ) = 
      &do_validation( 'password',
                      $myhash{'password'},
                      $CMS::User::data_topo{usert}{'password'},
                      $hash );
    if ( ! $ok ) {
      # handle bad value
...
However, since the CMS system has been planned from the
