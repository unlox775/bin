...
#########################
###  Dave/Util.pm
###  Version : $Id: Util.pm,v 1.1 2010/11/17 23:34:19 dave Exp $
###
...
sub binary_search {
  ...
  my ( $last_value, $higher ) = ( -10000000000, 1);
...
#    print STDERR sprintf("At %.4f percent (%.0f, range is %.0f)...\n", $middle / $fsize, $middle, $rend - $rstart) if $debug;
    ...
    READ : while ( $tell < $rend ) {
      ...
      my ($value) = ($line =~ /(^\d+)/);
      if ( defined $value ) {
        return $line if $value == $goal;
        if ( $first_line ) {
          die "File is not sorted, expected lower value than $last_value, got $value" if !$higher && $value > $last_value;
          die "File is not sorted, expected higher value than $last_value, got $value" if $higher && $value < $last_value;
        }
...
Util - Lib for shared utility functions

=head1 SYNOPSIS

    use Util qw(&split_delim_line &join_delim_line);
    ...
    use Util qw(&read_conf &write_conf);
    ...
    use Util qw(&do_validation);

    my ( $ok, $newval, $error ) = 
      &do_validation( 'password',
                      $myhash{'password'},
                      $User::data_topo{usert}{'password'},
                      $hash );
    if ( ! $ok ) {
      # handle bad value
