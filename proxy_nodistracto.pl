...
#    print "DEBUG: Parent forked process [$pid]\n";
  ...
  if ($REQUEST=~/^Content-length: (\d+)/im) {
    ...
    $REQUEST.=$CONTENT;
##    print "=================Should be printing to /home/dbuchanan/PROXY.LOG=======================\n\n";
    open (LOG,">>/home/dbuchanan/PROXY.LOG");
    print LOG "\n<--------------------------[$LOGCOUNT]\n$REQUEST";
    close LOG;
  }
  print "DEBUG: REQUEST-------------------------------------------\n$REQUEST\n";
  ...
  next REQUEST unless $REQUEST =~ /d?cms.prosperlearning.com|browsersync.google.com|google.com\/mail/;
